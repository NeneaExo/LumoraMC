...
import { authenticate, isAdmin, isStaff, isMod, authenticatePlugin } from "../middleware/auth";
...
// ==================== RULES (Regulament) ====================
router.get("/rules", rules.getRules);                                          // Public
...
// ==================== ANNOUNCEMENTS ====================
router.get("/announcements", announcements.getAnnouncements);                  // Public
router.get("/announcements/:id", announcements.getAnnouncement);
router.post("/announcements", authenticate, isMod, announcements.createAnnouncement);
router.put("/announcements/:id", authenticate, isMod, announcements.updateAnnouncement);
router.delete("/announcements/:id", authenticate, isMod, announcements.deleteAnnouncement);

// ==================== MODERATION ====================
router.get("/moderation", authenticate, isStaff, moderation.getActions);
router.get("/moderation/recent", authenticate, isStaff, moderation.getRecentActions);
router.get("/moderation/player/:username", authenticate, isStaff, moderation.getPlayerHistory);
router.post("/moderation", authenticate, isMod, moderation.createAction);

// ==================== MINECRAFT PLUGIN (autentificare prin API key) ====================
router.post("/minecraft/players/sync", authenticatePlugin, minecraft.syncPlayers);
router.post("/minecraft/players/:uuid/join", authenticatePlugin, minecraft.playerJoin);
router.post("/minecraft/players/:uuid/quit", authenticatePlugin, minecraft.playerQuit);
router.post("/minecraft/stats", authenticatePlugin, minecraft.updateServerStats);
router.post("/minecraft/staff/:username/status", authenticatePlugin, minecraft.updateStaffStatus);
router.post("/minecraft/moderation", authenticatePlugin, minecraft.reportModerationAction);

// ==================== MINECRAFT DATA (citire — staff autentificat) ====================
router.get("/minecraft/players", authenticate, isStaff, minecraft.getOnlinePlayers);
router.get("/minecraft/stats/history", authenticate, isAdmin, minecraft.getStatsHistory);
